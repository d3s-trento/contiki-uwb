...
uint8_t* app_pre_T() {
    if (app_have_packet) {
		...
		for (int i=0; i< PAYLOAD_LENGTH; ++i) {
			t_payload.payload[i] = (uint8_t)((uint8_t)t_payload.seqn + (uint8_t)i);
		}
...
uint8_t* app_between_TA(int received, uint8_t* payload,  const crystal_data_hdr_t * const data_hdr) {
    if (received) {
		...
		bool ok = true;
		for (int i=0; i< PAYLOAD_LENGTH; ++i) {
			uint8_t tmp = (uint8_t)((uint8_t)t_payload.seqn  + (uint8_t)i);
			if (t_payload.payload[i] != tmp) {
				ok = false;
				printf("WARN: is %" PRIu8 " should be %" PRIu8 "\n", t_payload.payload[i], tmp);
			}
		}
		if (!ok) {
			printf("WARN: not ok %" PRIu16 "\n", data_hdr->src);
		}

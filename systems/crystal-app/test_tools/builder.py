...
version_map = {"no": 0, "aggr": "CRYSTAL_VARIANT_AGGR", "cons": "CRYSTAL_VARIANT_CONS", "simple": 3}

def mk_env(power, radio_cfg, sink, num_senders, n_empty, crystal_variant):
    cflags = [
    ...
    "-DCRYSTAL_VARIANT=%s"%version_map[crystal_variant],
    "-DNODES_DEPLOYED=\"%s\""                % str.join(",", map(str, sorted(all_senders))),# no space after comma, or Make will complain :(
    ]
    ...
    cflags += ["-DSTART_DELAY_SINK=5", "-DSTART_DELAY_NONSINK=2"]
...
for (power, sink, num_senders, n_empty, crystal_version) in itertools.product(powers, sinks, num_senderss, n_emptys, crystal_versions):
    ...
    try:
        ...
        if all_senders is None:
            raise Exception("All senders not set")

        print(f'All senders: {all_senders}')

        if (num_senders > len(all_senders)):
            raise Exception("More senders than nodes: %d > %d, skipping test"%(num_senders, len(all_senders)))

        new_env = mk_env(power, radio_cfg, sink, num_senders, n_empty, crystal_version)
        ...
        with open(os.path.join(simdir, "params_tbl.txt"), "w") as f:
            ...
            p["seed"] = seed
            p["crystal_version"] = crystal_version

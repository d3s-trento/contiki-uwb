...
SAMU_LOG         = r"(?i)^\[samu\s+(?P<epoch>\d+)\]Slots:\s+(?P<slots_desc>[_trleby#+\-0-9mp$~xd!]+)$"
IS_SINK         = r"(?i)^is_sink$"
...
LABEL_ACK          = "ack"
LABEL_CLOG         = "weaver_log"
LABEL_SAMU_LOG      = "samu_log"
LABEL_NSLOTS       = "nslots"
...
FILTER_RULES = OrderedDict([
    ...
    (re.compile(SAMU_LOG),           LABEL_SAMU_LOG),
...
def convert_samu_log(samu_string, nid, epoch, mismatch_slot=[]):
    ...
    if match:
        tmp = tmp[:match.end()-1]
        logger.warn(f"Found mismatch in samu string of node {nid} at epoch {epoch}. Processing is limited...")

    match = re.match(".*_(?P<first_slot>\d+)", tmp)
    if not match:
        # this can happen if we fail to bootstrap
        #raise ValueError("Couldn't find a sync slot")
        logger.error(f"Node {nid}, epoch {epoch}: Couldn't find a sync slot. SAMU log {samu_string}")
        return
    ...
    while len(tmp) > 0:
        ...
        if c in "TRY": # TX, RX AND Y (i.e. first success RX) are already tracked in clogs!
            ...
            raise ValueError(f"Unmatched element {c} at {len(samu_string) - len(tmp)} in {samu_string}")
...
def parse_log(filepath, parser, ignore_slots=True, info={}):
    ...
    for nid, log in parser(filepath):
        ...
        if rule == LABEL_IS_SINK:
            ...
            try:
                ...
                for entry in convert_samu_log(slots_str, nid, epoch, mismatch_slot=mmatch):
                    samu_logs.append(entry)
            except ValueError:
                logging.warning("Cannot use samu_slots")
    ...
    samu_logs_pd  = pd.DataFrame(samu_logs, columns=["node", "epoch", "slot_idx", "status"]).sort_values(["epoch", "node", "slot_idx"])
    ...
    return info, sent_pd, rcvd_pd, clogs_pd, nslots_pd, origs_pd, acks_pd, samu_logs_pd, errs_pd, mismatches_pd, statetime_pd, rstats_pd, boot_pd
...
def get_ack_rate2(sent_pd, rcvd_pd, n_orig, sink_id, log=True, savedir=None):
    ...
    if len(c2):
        info("Some packets have been received more than once.\n %s" % c2.head())
        rcvd_pd = rcvd_pd.drop_duplicates(['node', 'sender_id', 'seqno'])
...
def plot_epoch(cslots_fp, samu_slots_fp, nodes, bitmap_order, epoch, savedir=".", interactive=False):
    ...
    # integrate samu info
    samu_slots_pd = pd.read_csv(samu_slots_fp)
    samu_slots_pd = samu_slots_pd[(samu_slots_pd.epoch == epoch) & (~samu_slots_pd.status.isin(["T", "R", "Y"]))]

    # extend dataframe
    samu_slots_pd["distance"] = np.nan
    samu_slots_pd["sender_id"] = np.nan
    samu_slots_pd["lhs"] = np.nan
    samu_slots_pd["acked"] = np.nan
    samu_slots_pd["buffer"] = np.nan
    # align column order to that of cslots
    samu_slots_pd = samu_slots_pd[["node", "epoch", "slot_idx", "distance", "status", "sender_id", "lhs", "acked", "buffer"]]

    epoch_pd = pd.concat([epoch_pd, samu_slots_pd], axis=0, ignore_index=True) # concat on rows
...
def get_samu_error(samu_slots_fp, sink_id):
    samu_slots_pd = pd.read_csv(samu_slots_fp)
    slot_mask = (samu_slots_pd.status == "E") & (samu_slots_pd.node == sink_id)
    samu_slots_pd = samu_slots_pd.loc[slot_mask, ["epoch", "slot_idx"]]
    return samu_slots_pd

def get_stats_lastrx(sink_id, sim_epochs, cslots_fp, samu_slots_fp):
    ...
    err_pd = get_samu_error(samu_slots_fp, sink_id)
...
def run_log_parser(args):
    ...
    for log_file in args.log_files:
        ...
        samu_slots_fp = SAVEDIR.joinpath("samu_slots.csv")
        ...
        if not (sent_fp.exists() and sent_fp.is_file()) or\
           ...
           not (samu_slots_fp.exists() and samu_slots_fp.is_file()) or\
           not (errs_fp.exists() and errs_fp.is_file()) or\
           args.force is True:

            stats, sent_pd, rcvd_pd, cslots_pd, nslots_pd, origs_pd, acks_pd, samu_slots_pd, errs_pd, mismatches_pd, statetime_traces_pd, rstats_pd, boot_pd = parse_log(log_file, parser=povo_parser, ignore_slots=False)
            ...
            samu_slots_pd.to_csv (samu_slots_fp,  index=False)
        ...
        if args.plot:
            ...
            plot_epoch(cslots_fp, samu_slots_fp, nodes=stats.nodes, bitmap_order=stats.bitmap_order, epoch=args.plot,\
        ...
        print(origs_pd['epoch'].unique())
        print(ref_epochs_pd["epoch"].unique())
        ...
        print(f"min_epoch {min_epoch} max_epoch {max_epoch} offset {offset}")

        print(f"Epochs considered: {len(sim_epochs)}")
        print('ref_epochs_pd')
        print(ref_epochs_pd)
        ...
        err_pd   = get_samu_error(samu_slots_fp, simconf.sink_id)
        ...
        if not (origs_pd['n_originators'] == 0).all():
            ...
            if len(cslots_pd):
                stat_before_last = get_stats_lastrx(simconf.sink_id, sim_epochs, cslots_fp, samu_slots_fp)

...
E = re.compile(record_pattern % "E (?P<epoch>\d+) (?P<phase>\w) (?P<idle>\d+) (?P<tx_preamble>\d+) (?P<tx_data>\d+) (?P<rx_hunt>\d+) (?P<rx_preamble>\d+) (?P<rx_data>\d+)")
F = re.compile(record_pattern%"F (?P<epoch>\d+):(?P<tf_s>\d+) (?P<tf_t>\d+) (?P<tf_a>\d+):(?P<n_short_s>\d+) (?P<n_short_t>\d+) (?P<n_short_a>\d+)")
G = re.compile(record_pattern%"G\s+(?P<epoch>\d+)\s+(?P<phase>\w+)\s+(?P<slot_duration>\d+)\s+(?P<round_duration>\d+)\s+(?P<ntx>\d+)\s+(?P<nrx>\d+)")
...
def parse():
    ...
    Elog.write("epoch\tnode\tphase\tidle\ttx_preamble\ttx_data\trx_hunt\trx_preamble\trx_data\n")
    ...
    Glog.write("epoch\tnode\tphase\tslot_d\tround_d\tntx\tnrx\n")
    ...
    for fn in input_files:
        ...
        with open(fn, 'r') as f:
            ...
            curdir = Path(fn).parent.resolve()

            # copy nodelist and params table into test folder
            copy(str(SIMDIR.joinpath(NODELIST)), str(curdir))
            copy(str(SIMDIR.joinpath(PARAMS_TABLE)), str(curdir))
            ...
            for l in f:
                ...
                if m:
                        g = m.groupdict()
                        node, epoch, slot_d, round_d, ntx, nrx = tuple(map(int,\
                                                                           [g[f] for f in ["self_id", "epoch",
                                                                                           "slot_duration", "round_duration",
                                                                                           "ntx", "nrx"]]))
                        phase = g["phase"]
                        if phase == "#":
                            raise ValueError("Unknonw phase found, something very wrong happened.")

                        Glog.write("%d\t%d\t%s\t%d\t%d\t%d\t%d\n"%(epoch, node, phase, slot_d, round_d, ntx, nrx))
                        continue
                m = E.match(l)
                if m:
                        g = m.groupdict()
                        ...
                        phase    = g["phase"]
                        idle     = int(g["idle"])
                        tx_preamble = int(g["tx_preamble"])
                        tx_data  = int(g["tx_data"])
                        rx_hunt  = int(g["rx_hunt"])
                        rx_preamble = int(g["rx_preamble"])
                        rx_data  = int(g["rx_data"])

                        assert phase in "STAF"

                        Elog.write("%d\t%d\t%s\t%d\t%d\t%d\t%d\t%d\t%d\n"%(epoch, node, phase, idle, tx_preamble, tx_data, rx_hunt, rx_preamble, rx_data))
                        continue

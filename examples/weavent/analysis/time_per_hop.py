...
for LOG in args.log:
    ...
    ax.set_title(f'Latency divided by hop count ({LOG})')

    data = data.merge(epochData, on=['node_id', 'epoch'])
    data = data.sort_values('node_id')
    data['diff'] = data['diff'] / data['hop']
    ...
    ax.boxplot(data['diff'], sym='x', flierprops=dict(markersize=3, alpha=0.7))
    p = ax.violinplot(data['diff'], showextrema=False)

    ax.set_ylim([args.minTime, args.maxTime])

    print('Description of diff column')

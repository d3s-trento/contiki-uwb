...
def plot():
    ...
    for (label, _), color in zip(args.logs, colors):
        ...
        ax.boxplot(w, sym='x', whis=(0.01, 0.99), flierprops=dict(markersize=3, alpha=0.7), showfliers=False, medianprops=dict(color=color), widths=0.85, positions=hops)

        if not args.save:
            vp = ax.violinplot(w, showextrema=False, widths=0.85, positions=hops)
        ...
        ax.plot(hops, 1000*(fhl + lph*(hops)), linestyle='solid', label=f'{label}', color=color)

    ax.set_xlabel('Hop distance')
    ax.grid(alpha=0.2)
    ax.tick_params(left=True)  # re-add the ticks only for the left plot
    ax.tick_params(axis='y', labelrotation=55, pad=-1)
    ax.set_ylabel(r'Latency [\si{\us}]')

    # ax.legend(loc='upper center', ncol=2)#, bbox_to_anchor=(0.5, 1))
    # fig.suptitle(' ')
